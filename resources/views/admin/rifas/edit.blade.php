...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('admin.rifas.show', $rifa) }}" class="text-gray-600 hover:text-gray-900">
            ← Volver
        </a>
    </div>

    <div class="max-w-2xl">
        <form action="{{ route('admin.rifas.update', $rifa) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-8 space-y-6">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $rifa->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="start" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           value="{{ old('start', $rifa->start->format('Y-m-d')) }}">
                ...
                <div>
                    ...
                    <input type="date" name="end" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           value="{{ old('end', $rifa->end->format('Y-m-d')) }}">
            ...
            <div>
                ...
                <select name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="0" {{ old('status', $rifa->status) === '0' ? 'selected' : '' }}>Inactiva</option>
                    <option value="1" {{ old('status', $rifa->status) === '1' ? 'selected' : '' }}>Activa</option>
                </select>
